...
def load_warehouse_map() -> dict:
    with open('./data/warehouse.txt', 'r') as file:
...
magazyn_dict = load_warehouse_map()
...
def rysuj_kwadrat(lista_punktow):
    map_image = Image.open("/data/maps/mapa.jpg")
    draw = ImageDraw.Draw(map_image)
    for x, y in lista_punktow:
        draw.rectangle([x - 12.5, y-12.5, x + 12.5, y+12.5], fill=(255, 0, 0))
    map_image.save("/data/maps/pomoc.jpg")

def rysuj_prosta(lista_punktow, filename):
    map_image = Image.open("/data/maps/pomoc.jpg")
    draw = ImageDraw.Draw(map_image)
    ...
    map_image.save(f"/data/maps/{filename}")
# Otwieramy plik magazyn.txt do odczytu



def generate_path_image(filename):
    kroki = []
    with open('/data/maps/zapis_róchów.txt', 'r') as file:
        for line in file:
